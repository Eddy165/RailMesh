...
class SyntheticDataLoader(DataLoader):
    def __init__(self, seed: int = 42):
        ...
        self.segments_data = [
            ("NDLS_NGP", "NDLS", "NGP", 1094.0, 14, 840),
            ("NGP_MMCT", "NGP", "MMCT", 837.0, 10, 720),
            ("NGP_BZA", "NGP", "BZA", 571.0, 8, 480),
            ("HWH_NGP", "HWH", "NGP", 1131.0, 12, 900),
            ("BZA_MAS", "BZA", "MAS", 431.0, 10, 360)
        ]
    ...
    def load_segments(self) -> List[TrackSegment]:
        return [
            TrackSegment(
                ...
                length_km=length, capacity=cap, travel_time_mins=time
            ) 
            for seg_id, src, tgt, length, cap, time in self.segments_data
        ]

    def load_trains(self) -> List[Train]:
        return [Train(id=tid, name=name, priority_class=p_class) for tid, name, p_class in self.trains_data]

    def load_schedules(self) -> List[TrainSchedule]:
        now = datetime.now(timezone.utc)
        schedules = []
        
        # Mumbai Rajdhani (NDLS -> NGP -> MMCT)
        ...
        t2_f = t1_f + timedelta(minutes=1000) # Slower
        schedules.append(TrainSchedule(
            train_id="F8492",
            route=["HWH_NGP"],
            entries=[
